...
- **`Mainguard.UI`** — design system + the four themes + the edition seams (`IEditionManifest` / `IAgentPlatformSurface`) + `ViewModelBase` + `ViewLocator`.
...
- **No raw colors in UI.** Bind design tokens with `{DynamicResource …}` (never `StaticResource` for colors — it won't follow live theme switches). Pick a `Button.*` / `Border.*` component class by role instead of setting `Background`/`Foreground`. New tokens go in **every** `Themes/*.axaml`; new component classes go in `Mainguard.UI/Styles/DesignSystem.axaml`, new icons in `Mainguard.UI/Styles/Icons.axaml`. There is one design system with four switchable color themes — never assume "dark" (Daylight Loom is light).
...
- **Check `packages.lock.json` before staging.** The daemon-graph projects (`Mainguard.Server`, `Mainguard.Agents`, `Mainguard.Git`, `Mainguard.Protos`) DECLARE `<RuntimeIdentifiers>linux-x64;win-x64;osx-arm64</RuntimeIdentifiers>`, so every restore — plain, RID'd, partial — evaluates identically and their committed lockfiles stably carry all three RID sections (this retired the old per-machine flapping where a WSL build added `linux-x64` or a Mac added `osx-arm64`). A lockfile diff now means a real dependency change; CI restores `--locked-mode`, so an unintended one still breaks loudly. `git checkout -- '**/packages.lock.json'` if you did not mean it.
