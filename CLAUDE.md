...
- **Check `packages.lock.json` before staging.** The committed lockfiles carry `linux-x64`/`win-x64`/`osx-arm64` RID sections; a PARTIAL-graph restore (building one project) silently DROPS the sections its subgraph doesn't need, and a RID build on a new platform silently ADDS one. CI restores `--locked-mode`, so either direction breaks a build or quietly changes the supply-chain pin looking intentional. `git checkout -- '**/packages.lock.json'` if you did not mean it; prefer whole-solution restores (or `--no-restore` partial builds).
