...
- **`Mainguard.Server.Tests/QueueDisplayOrderTests.cs`** — unit tests (no daemon, no Docker) for
  ...
  list — the exact "spawned agent isn't in the queue" symptom. Extended 2026-08-22 for the mirror-image
  defect (ISSUES-LOG #13): the terminal group is ordered newest-decision-first, so a branch just
  rejected heads the history instead of sorting behind every older verdict; a terminal row with no
  decision timestamp keeps its insertion position at the back and is never dropped. The end-to-end
  positional assertion lives in `QueueEntryLifecycleTests.RejectEntry_PutsTheFreshVerdictAtTheHeadOf…`.
...
- **`Mainguard.Tests`** — xUnit tests for Core + App + the client-side daemon pieces; hosts the
  ...
  **`WorkerPlanGateTests` also carries the phase-3 key** — held tasks are keyed by `(RepoHash, AgentId)`,
  ...
  repo's withheld task, with a paired positive proving an unambiguous id still releases normally. The
  release-exactly-once pair above and this key interact — the `Released` write-back must use the COMPOSITE
  key, or it inserts a second entry instead of latching and silently un-fixes the idempotence,
...
- **`Mainguard.Server.Tests`** (P2-02 / TI-P2-00) — the daemon in-proc test tier
  ...
  **`JailReaperTests` (2026-09-04 — the reaper over the real composition root: a jail whose fake CLI is
  ...
  blanket reprieve), and the mark is cleared by the first sweep that sees a bound CLI. Plus audit F20: a session the reconciler has marked `Unresponsive` is NOT reaped, at any
  ...
  **Audit W2D (daemon host + auth) tests:**
  ...
  `ReadOnlyAttachTests` (**F64** — a lock applied MID-attach is honoured on the next frame, i.e. the
  lock is read live rather than snapshotted; `TryClaimInput` is exclusive and released on dispose; and
  concurrent `WriteInputAsync` calls are not interleaved at the byte level. **MG-24 narrowed F64 by the
  owner's decision — the lock is about INPUT, and a locked attach resizes** — so the old
  "cannot Resize" test is inverted into `LockedAttach_CannotType_ButCanStillResize`, which asserts both
  halves in one place precisely so a later change cannot trade one for the other: the DATA frame is
  still `PERMISSION_DENIED` and never reaches `WrittenText`, while the Resize DOES reach the CLI.
  Geometry is then pinned on top: the `geometry` frame leads the stream and reports the size the
  session was BOUND at rather than anything the attach asked for, and both a locked and an unlocked
  attach are told the NEW size once their resize lands — read by scanning for the frame rather than by
  position, since raw CLI frames interleave with it),
  ...
  **Skipped** with a reason. **`[RequiresDockerDesktopFact]` and `[RequiresLinuxDockerEngineFact]`**
  split the claims that differ by engine, because `host-gateway` means the host's loopback stack on
  Docker Desktop and the docker0 address on native Linux — a test asserting one of those
  unconditionally fails the other platform's CI leg by design (audit B3). All the probes are cached in
  `DockerAvailability`; class-level `[Trait("Category","RequiresDocker")]` carries the CI filter, and
  `FixtureAcceptanceTests` cross-checks the trait against any `RequiresDocker*` attribute by name, so
  the new ones count),
