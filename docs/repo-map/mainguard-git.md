...
- **`Security/`** — `SecureKeyring.cs` (OS keyring / DataProtection secret storage; T-14 added a
  ...
  The passphrase may also be supplied as a FILE — `MAINGUARD_KEYRING_PASSPHRASE_FILE` names a path
  (trailing newline stripped), which is how the shipped `mainguardd.service` hands it over and how a
  systemd `LoadCredential=` is consumed: an `Environment=` secret is readable via `systemctl show`,
  `/proc/<pid>/environ` and every child process, a path is not.
  `KeyringRingMaintenance.cs` — **B2**: configuring an `IXmlEncryptor` only ever protects *newly
  generated* keys, so an install that predates the protector kept its plaintext `key-*.xml` and kept
  using it as the DEFAULT key until it expired 90 days later; an operator who set the passphrase on an
  upgraded box got no protection and no warning. The ring is now inspected at every open and
  **re-wrapped in place** (same key id, same key material, so every stored secret keeps decrypting —
  a re-wrap, not a rotation), with a decrypt-back self-check before anything is written and an atomic
  temp-file replace. The outcome is a `KeyringRingReport` (`KeyFiles` / `PlaintextKeyFiles` /
  `RewrappedKeyFiles` / `MasterKeyUnavailable` / `Detail`) surfaced as `SecureKeyring.Ring` +
  `DescribeProtection()`, which the daemon logs at boot — before this, `Protection`/`IsUnprotected`
  had **zero callers** outside the file. Two consequences: a ring that is *still* plaintext under a
  configured protector refuses the fail-closed keys just as an unprotected platform does; and a ring
  encrypted under a protector this process cannot use (passphrase missing, Keychain locked) reports
  `MasterKeyUnavailable` and refuses them with `KeyringProtectorUnavailableException` — because
  DataProtection's own answer to an undecryptable ring is to mint a FRESH default key (unencrypted, on
  a platform with no protector), so the write would have "succeeded" into plaintext and orphaned every
  payload the old key protected. `ResolveProtection()` is `internal` (the daemon test tier asks it
  which posture a machine can achieve) and macOS now resolves to `None` when the login Keychain will
  not serve the master key, since `MacKeychainXmlEncryptor` fails OPEN and reporting `MacKeychain`
