...
> **ARCHIVED — the rename landed.** The repo, the solution and every identifier are `Mainguard.*`
> today, so the phases below are history, not a plan. Two cautions if you read on: a later blanket
> rename pass rewrote the old product name *inside this file*, so its title now reads as a no-op;
> and the theme renames it proposes were never applied (the themes are Midnight Loom, Daylight
> Loom, Graphite, Atelier).
