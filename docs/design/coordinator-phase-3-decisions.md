...
## 20. Defects L2 and L4 — the audit chain did not record the merge, or the waiver that allowed it
...
### 20.1 What was observed, with ground truth
...
### 20.2 Which points in the merge conversation deserve an event
...
### 20.3 Where `queue_entry_merged` is emitted, and why not at the RPC
...
### 20.4 What the record carries, and why each field is there
...
### 20.5 What happens when the audit store is down
...
### 20.7 Left alone, deliberately

- **`BeginMerge` is not audited.** See §20.2. Its grant is named by the merge record's `lease` field.
