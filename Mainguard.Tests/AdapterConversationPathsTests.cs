...
/// <summary>
/// The <c>conversationPaths</c> declaration (and its pairing with the existing <c>resumeArg</c>), and above all the ONE invariant that
...
public class AdapterConversationPathsTests
{
    ...
    [Fact]
    public void ConversationPaths_AndResumeArg_ParseAndAreReadable()
    {
        ...
        Assert.Equal("--continue", a.ResumeArg);
    }
    ...
    [Fact]
    public void AResumeArg_WithoutConversationPaths_IsACCEPTED()
    {
        // Deliberately the OPPOSITE of what a conversation-store-only reading would want, and the
        // reason is a second consumer that predates this feature. SandboxAgentLauncher's
        // BuildReattachLaunchArgv passes the same resumeArg when a daemon restart re-binds a CLI into a
        // jail that is STILL RUNNING: there the tmpfs $HOME never died, the transcript is already in
        // the jail, and no store is involved at all. Refusing this manifest would break that path.
        //
        // conversationPaths is what makes the SAME flag mean something once the jail itself is gone.
        // The two are independently true, so neither may be made to imply the other.
        var a = Assert.Single(AdapterManifest.Parse(Manifest(Adapter(
            """
            "resumeArg": "--continue"
            """))).Adapters);
        ...
        Assert.Equal("--continue", a.ResumeArg);
        Assert.Null(a.ConversationPaths);
    }
    ...
    public void TheBundledManifest_DeclaresProjectsForClaudeCode_AndNeverDotClaude()
    {
        ...
        Assert.Equal("--continue", claude.ResumeArg);
