...
This file provides guidance to ChatGPT / OpenAI Codex when working with code in this repository.

## Read AGENTS.md first

**[`AGENTS.md`](AGENTS.md) is the source of truth** for architecture, the design system, conventions, and git hygiene, and it is kept current with the code. The OpenAI Codex CLI already loads `AGENTS.md` natively, so it is the primary, most detailed instruction file — read it before changing anything. This file only adds operational specifics and highlights a few rules you must not miss. Where the two ever disagree, AGENTS.md wins — and fix the drift.
...
- **Check `packages.lock.json` before staging.** A WSL build silently adds a `net10.0/linux-x64` target to three committed lockfiles with no `PackageReference` change. CI restores `--locked-mode`, so it either breaks a build or quietly widens the supply-chain pin looking like an intentional change. `git checkout -- '**/packages.lock.json'` if you did not mean it.
