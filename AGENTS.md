...
**[`docs/STATUS.md`](docs/STATUS.md) answers "is X done?"** — one row per master-doc task, with the file that proves each claim. This document defines the tasks; it does not record their status. **A PR that finishes a task updates its row there in the same PR**, on the same rule as the repo map.
...
| `docs/planning/` | **`Mainguard_Backlog.md` only — the single source of truth for what is *not* built yet.** The phase-1 planning corpus that used to live here (the v1 roadmap, implementation plan, implementation strategy and master implementation document, the foundation audit, `Product_Editions_And_Structural_Sequencing.md` — shipped, see ADR-0001 — and `Agent_Image_Provisioning_And_Daemon_Logging_Backlog.md`, whose two items are both marked ADDRESSED 2026-07-18) moved to `docs/archive/planning/` once its work shipped. |
...
| `docs/creative/` | Voice/brand/writing output. **`README.md` is the brand & voice hub — start there** (authority ladder; consolidated 2026-07-12). The Voice & Delight Bible is the law (now incl. **Appendix C** agent naming + **Appendix D** release-notes voice, folded verbatim from the archived LaunchReserve); `Microcopy.md` owns final strings (incl. §6 empty-state strings), `EmptyStates.md` the ES-card layout spec, `MotionPlaybook.md` per-moment motion briefs (read with DesignSystem Part 4), `Onboarding.md` the first-run storyboard, `PolishSpec.md` the live polish punch-list, `ThemeRefinement.md` the color-retune spec (§4.1 superseded by DesignSystem Part 1 — authority note at its top), `Narrative.md` the brand-register market/launch layer of record (honesty contract §0). `LaunchReserve.md` → archived at `docs/archive/`. |
| **`docs/BUSINESS.md`** | **The business strategy doc — start here for anything non-code.** What we sell and refuse to sell, the shipped-vs-[Horizon] product and the D-1…D-6 differentiation spine, market numbers, the competitive field, ICP/personas/positioning, the objection kit (concede→fact→line), pricing and cloud unit economics, the five-layer moat with its erosion dashboard, risk registers, and metrics. Consolidated 2026-09-16 from the 33-doc business corpus. |
| **`docs/GTM.md`** | **The go-to-market execution doc.** The stage rule (beta-feedback recruitment, Show HN held in reserve behind a working spawn→verify→review→merge gate), the two-act launch sequence, the founding-user and design-partner programs, channels and paid-media ground rules, the sales motion and sourcing plays, the NL grant stack and equity track, Dutch company setup and hiring, and the gates-before-dates calendar. |
| **`docs/GTM_Assets.md`** | **The reusable copy, as templates to update — never as finished artifacts.** The one-pager, the investor deck (slide copy + speaker notes) and its three variants, the 10-minute demo script, outreach sequences (EN/NL), site and email copy, the scored target-company list (dated 2026-08-20 — re-verify before every send), WBSO/VFF/MIT application drafts, the Show HN bodies and first-hour FAQ, and press/video material. |
| **`docs/business/NL_Operations.md`** | **Dutch lookups — reference, not reading.** Grant schemes and their rigid deadlines (WBSO before the month work starts; MIT one day in April), the Dutch fund and angel landscape, BV incorporation and the VAT/merchant-of-record decision, Twente salary bands and the internship/ESOP picture, and the press and events map. Open it when filing, incorporating, hiring or pitching a Dutch outlet; nothing in it is needed to understand the product. |
| `docs/business/` | **The research corpus behind the business docs above** — `go-to-market/` (master market document, decks, marketing, sales, grants) and `market-analysis/` (competitor and market research, with the full source lists). Consolidated 2026-09-16: **read the summary first**, follow its per-section *Detail:* links down here for depth, and where the two disagree the summary wins. Every file carries a banner saying so. |
| `docs/business/members/` | Team structure and intake. |
| `docs/review/` | Human review guides — what to review, how, and what "correct" looks like: the phase-1/2/3 hands-on procedures, `agent-cycle-runbook.md`, `full-test-matrix.md`, and the manual `Mainguard_User_Testing_Guide.md`. Dated walkthrough sessions are archived under `docs/archive/walkthroughs/`. |
| `docs/archive/` | **Nothing here is current** — read it for provenance, never as a spec. Start at its [`README.md`](docs/archive/README.md). Holds: `feature-plans/phase-2/` (the 16 `P2-01`…`P2-22` plans, **all shipped** — each verified against the code before archiving), `feature-plans/git-core/` (the retired `T-02`…`T-32` plans), `planning/` (the phase-1 corpus), `reports/` (point-in-time snapshots), `walkthroughs/` (dated review sessions), the v1 test strategy, the completed `Mainguard_Rebrand_Plan.md`, `LaunchReserve.md` (supersession map in its header), and `prompt-bundles/`. Every archived doc carries an `ARCHIVED` banner under its title saying why. Renamed from `obsolete docs/` (the old name contained a literal space, which broke relative links). |

**README.md** (repo root) is the product overview (current vs. planned features). **CONTRIBUTING.md** (repo root) is the contributor on-ramp — build/test/run, where code goes, the five hard review rules, testing conventions — and defers to this file wherever they could disagree. **CLAUDE.md**, **GEMINI.md** and **CHATGPT.md** (repo root) are thin per-assistant pointers at this file and carry no rules of their own — keep them that way, so this file stays the only one that can go stale.
...
- **A vendor CLI's launch flags are VERIFIED per adapter, never inferred from a neighbour.** `systemPromptArg`, `preApprovedCommandArg`, `initialPromptStyle`, `modelArg` — each is read out of that CLI's own argument parser in the exact pinned artifact the manifest installs (its `--help`, its clap/yargs table, or the binary's own strings). A wrong flag does not degrade: it fails every spawn of that kind with a vendor error message that reads as a Mainguard bug. "Most of these CLIs take `--model`" is not evidence for any one of them.
- **A new adapter field must go through `InstalledAdapterMarker.FromSpec`, or it is inert on every real install (defect D5a).** A marker is a file written the day the CLI was installed, so it predates your field; `WithShippedDescription` = `FromSpec(spec) with { Version, Launch }` is what backfills it. Two shipped fixes were already completely dead this way while every test stayed green — pin the projection with a test.
- **An agent's `Name` is its CLI kind, not its identity.** Four `claude-code` sessions produce four identical values, so anything a human reads binds `AgentInfo.DisplayName` — the name they typed, else the worker's brief (taken from the plan stream, never a new wire field), else `AgentRoles.Word(role) + AgentInfo.ShortId(id)` — and never `Name` or a bare 32-hex id. `AgentRoles.Word` and `AgentInfo.ShortId` are the ONE home for that vocabulary: the short id is how a person matches an agent across the rail, the resource monitor, the merge queue and a plan card's "Written by …", so two surfaces shortening it differently breaks the match.
- **Ending an agent and deleting one are different acts, and must stay different methods.** `EndAgentAsync` / `StopAgent` tears the sandbox down and KEEPS `agent/<id>`, because that ref is the only name the agent's commits have. `DeleteAgentAsync` / `DeleteAgent` destroys it. Never collapse them into a flag: they are separate methods because `RoleInterceptor` denies by METHOD (deleting is coordinator-denied and stopping is not), and because a confirmation for the first is not consent to the second. A delete reports the sha it destroyed — the mirror's reflog is the only way back from a mistaken one.
...
[`docs/archive/LaunchReserve.md`](docs/archive/LaunchReserve.md) as of 2026-07-12 —
...
`docs/GTM_Assets.md` §9 (archived original: `docs/business/go-to-market/marketing/SocialLaunchReserve.md`).
...
`docs/BUSINESS.md` §4 and the research corpus at `docs/business/`), the enemy framing ("the blind merge") and locked
