...
| `docs/archive/` | Superseded docs, kept in full for history: the retired git-core feature plans (`T-02`…`T-32`), feature-plan triage, `LaunchReserve.md` (archived 2026-07-12; supersession map in its header — §4/§6 live on as Voice Bible Appendices C/D, launch copy in `Narrative.md`), and `prompt-bundles/` (one-off agent run scripts, formerly loose at the repo root). **Nothing here is current** — read it for provenance, never as a spec. Renamed from `obsolete docs/` (the old name contained a literal space, which broke relative links). |
