...
| `docs/business/` | **The research corpus behind the three business docs above** — `go-to-market/` (master market document, decks, marketing, sales, grants) and `market-analysis/` (competitor and market research, with the full source lists). Consolidated 2026-09-16: **read the summary first**, follow its per-section *Detail:* links down here for depth, and where the two disagree the summary wins. Every file carries a banner saying so. |
...
`docs/BUSINESS.md` §4 and the archived research at `docs/business/`), the enemy framing ("the blind merge") and locked
